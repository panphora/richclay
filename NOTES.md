...
`removeCode()` calls `fixContainer(pre, root)` to repair the root it is about to splice the `<pre>` out of, but that call returns immediately: `fixContainer`'s guard regex `/^(?:TABLE|TBODY|TR|TH|TD|P)/` has no end anchor, so `"PRE"` matches on the `P`. The repair never happens, and the `<pre>`'s children land directly on the root. Present on Squire's current master as well as the vendored 2.4.8. richclay works around this one from its own side, without touching `vendor/squire.js`.

### Re-vendoring Squire

`npm run vendor:squire` (`scripts/vendor-squire.mjs`) is the only supported way to update `vendor/squire.js` and `vendor/squire.mjs`. It copies both dist files out of `node_modules/squire-rte`, applies the patches below to each, and writes `vendor/squire.provenance.json`. To bump Squire: change the pinned `squire-rte` version in `package.json`, `npm install`, `npm run vendor:squire`, `npm run build`.

There are two vendored builds and they are not interchangeable. `vendor/squire.js` is the IIFE a page loads with a `<script>` tag and the one the tests evaluate; `vendor/squire.mjs` is what `src/vendor-bundle.js` imports, so it is what ends up in `dist/richclay.min.js` and in every copy `scripts/propagate.js` writes. Patching one and forgetting the other leaves the suite green while the artifact real users load still has the bug, which is exactly what happened in round 5.

### Patches applied to the vendored Squire

Three of Squire's ancestor walks climb past the editor's own root. With a block root they stop naturally; with an inline root, which is richclay's whole inline mode, they walk out into the author's page. Clear formatting on a `<span editable>` moves every character outside the region, Unlink deletes an `<a>` the region merely sat inside, and select-all then Backspace deletes the region element itself. Upstream: https://github.com/fastmail/Squire/issues/501.

- `removeAllFormatting-root-guard` — `for (; n && n !== e && !isBlock(n) ;)`
- `removeFormat-root-guard` — `for (; r !== this._root && isInline(r) ;)`
- `removeEmptyInlines-root-guard` — `for (; n !== i._root && isInline(n) && … ;)`

Each patch records the line in upstream's readable `dist/squire-raw.js` it came from, so a bump that moves them can be re-derived from source rather than from the minified blob. The script refuses to write when a pattern does not match exactly once, and prints which patch, what it does, and where to look.

`tests/shipped-bundle.test.js` is the safety net, and it asserts the three *behaviours* against the built `dist/richclay.min.js` rather than the text of a patch. That catches a patch that missed a file, a stale build, and a bad propagate, and it keeps passing if upstream ever fixes this properly and the local patches are dropped.

### `vendor/squire.provenance.json`

Generated by `npm run vendor:squire` and committed. Never hand-edit it. It records the `squire-rte` version, which patches were applied, and the sha256 of each vendored file both as upstream shipped it and after patching, so "which Squire is this" and "has anyone hand-edited it" have answers in the repo. For the record, `vendor/squire.js` before round 5 was byte-identical to `squire-rte@2.4.8` `dist/squire.js` (sha256 `75c1665d33ec8c1a…`): it had never been hand-edited.
